...
import * as Perso from './perso.js';
                        ...
                        // référence globale de la carte (null tant qu'elle n'est pas créée)
...
( function () {
    ...
    //modif JPH pour formatage des dates
...
//modif jph ajout
    //*************** */
    function transmissionajax2(type,message)
    {//transmet un objet en le transformant en chaine
        
                    
                    var url= 'http://127.0.0.1:5000/api/ajaxmessage';
                    console.log ('Transmission du message '+type +' au serveur '+url )

                    fetch(url,{
                        method:'POST',
                        headers:{'Content-Type':'application/json'},
                        body: JSON.stringify({type: type, message:message})  //envoie les donnees json sous forme de string
                    })

                    .then(response => response.json())
                    .then(data => {
                        console.log('Données bien envoyees')
                        console.log(data.resultat)
                    })
                .catch(error=>console.log('Erreur: Connexion impossible));'));
...
// fin ajout 
    ...
    var divPositionInfo, divRaceStatus, divRecordLog, divFriendList, divWaypointsList, divRawLog, divVRRanking,divVrouteur /*, divClsmntList*/;
    ...
    function commonTableLines(r) {
		...
		var comfort = (r.curr.options.includes('comfortLoungePug')) ? 0.8 : 1;
		var stam = r.curr.stamina;
		var rtws = (r.curr.tws >=30) ? 1 : bezier(r.curr.tws / 30);
		var timeTo100 = ( stam >= 100 ) ? "" : '&nbsp;'+(( 100 - stam ) * (5 + 10 * rtws * comfort)).toFixed(0);
    ...
    function manoeuveringPenalties (record) {
		...
		if (options.includes('vrtexJacket')) {
			if ( tws < 10 ) {
				fraction2 = 1;
			} else if ( tws >= 10 && tws < 20 ) {
				fraction2 = 1 + ( tws - 10 ) / 10 * .2;
			} else if ( tws >= 20 && tws < 30 ) {		
				fraction2 = 1.2 + ( tws - 20 ) / 10 * .6;
			} else { 		
				fraction2 = 1.8;
			}
		} else {
        ...
        return {
            "gybe" : penalty(speed, options, fraction1, fraction2, fraction3, winch.gybe, 10, stam, 'G'),
            "tack" : penalty(speed, options, fraction1, fraction2, fraction3, winch.tack, 10, stam, 'T'),
            "sail" : penalty(speed, options, fraction1, fraction2, fraction3, winch.sailChange, 20, stam, 'S')
        };
    }
    ...
    function penalty (speed, options, fraction1, fraction2, fraction3, spec, fatig, stam, type) {
	//	console.log(speed +' '+ options +' '+ fraction1 +' '+ fraction2 +' '+ fraction3 +' '+ spec +' '+ fatig +' '+ stam +' '+ type) // PhLog
		...
		var fraction4 = 1;
		var fraction5 = 1;		
		// Le magicFurler n'agit que sur les changements de voiles
		if (options.includes('magicFurler') && type == 'S') {
			fraction4 = 0.8;
		}		
		if (options.includes('comfortLoungePug')) {
			fraction5 = 0.8;
		}
		...
		console.log(spec.lw.timer +' '+ spec.hw.timer +' '+ fraction1 +' '+ fraction4 +' '+ stam +' '+ type) // PhLog
		/* manoeuvre */
        var time = spec.lw.timer + (spec.hw.timer - spec.lw.timer) * fraction1;
		time = time * ( 2 - stam / 100 * 1.5 ) * fraction4; // * boatStaminaCoeff ????
        var dist = speed * time / 3600;
		console.log(boatStaminaCoeff +' '+ fatig +' '+ fraction2 +' '+ fraction4 +' '+ stam +' '+ type) // PhLog
		/* épuisement */
		var energyLoss = boatStaminaCoeff * fatig * fraction2 * fraction4;
		console.log(fraction3 +' '+ fraction5 +' '+ stam +' '+ type) // PhLog
		/* récupération */
		var recover = energyLoss * ( 5 + 10 * fraction3 ) * fraction5;
		console.log(spec.lw.ratio +' '+ stam +' '+ type) // PhLog
        return {
            "time" : time.toFixed(),
...
// --------- Intégration page sur VR ------------------
 ...
 divVrouteur = document.getElementById("Vrouteur");
 divVrouteur.classList.add('Vrouteur');
    ...
    function getRaceLegId (id) {
        ...
        if (id.raceId) {
            ...
            if (id.leg_num) {
                ...
                console.log("Unknown race id format" +id.raceId );
                console.log('id.legNum'+id.legNum)
    ...
    function tableClick (ev) {
        ...
        if (rmatch) {
            if (tabsel) {
                ...
                else if (rmatch == 9) {                             // On glet vrouteur
                setTimeout(() => {
                    if (!window._vrouteurMap) {
                    // Création unique
                var race = races.get(selRace.value);
                    var parametre='TEST JPH'
                    map =initializeVrouteur(race);
                    } else {
                    // Juste redimensionner si la carte existe
                    window._vrouteurMap.invalidateSize();
                    }
                }, 100);
                }
            ...
            //     setTimeout(() => {
            //     makeVrouteur(); // ici on crée la carte une fois que la div est affichée
            //     }, 50);
    ...
    function changeState(lbl_tochange) {
        //jph
        console.log('changeState label '+lbl_tochange)
...
//modif jph   // a eventuellement revoir avec ajaxmessage2
...
function transmissionajax(message)
{
...
catch{ rank=0    // on est dans le cas d un record 
     }
    ...
    if (tsEndOfGybe      ===undefined )       {tsEndOfGybe=lastCalcDate}        //  s'il n'y a pas eu de manoeuvre
...
//fin de modif jph  
    ...
    function callPolars(raceId) {
        ...
        if(cbBSPDown.checked){
            ...
            var IdRace = raceId ;
			var baseURL = "http://blacksailingpolars.vrzen.org/marina.htm?race_id=" + IdRace;
        }
...
function switchMapL(race) {
        initializeVrouteur(race);
        // races.forEach(function (r) {
        //     if (r.gdiv) {
        //         if (r == race) {
        //             r.gdiv.style.display = "block";
	...
	async function initializeMap(race) {
        ...
        if (!race.gdiv) {
            ...
            google.maps.event.addListener(map, "rightclick", function (event) {
                ...
                console.log ('lat0 '+lat0+' lng0 '+lng0+' paslat '+pasLat+' pasLng '+pasLng+' deltaLat '+deltaLat+' deltaLon '+deltaLon)
...
/*    var colors = [];
    ...
    function updateMapMeL(race,track){
         if (!race) return;
         ...
         if (track) {
            for (var i = 0; i < track.length; i++) {
                var segment = track[i];
                var pos = [segment.lat, segment.lon];
                tpath.push(pos);

            }
        // var posEnd=    [segment.lat, segment.lon];
        // console.log ('tpath ')
        // console.log (tpath)

        }
   
        // var heading = race.curr.heading
        // console.log('heading '+race.curr.heading)
        // console.log('pos     '+race.curr.pos)

        //  var map = race.lmap
        //  var boat= race.lboat
      
       // L.polyline(tpath) .setStyle({ color: 'black', weight:1, opacity:1, }).addTo(map); 

        // updateBoatMarker(boat, posEnd[0],posEnd[1], heading, 'red')
...
function buildlogBookHTML(race) {
    ...
    var raceIdentification = '<br><table id="raceidTable">'
        ...
        + '<thead>'
        + '<tr>'
        + '<th colspan="11">Credits <span style="color:limegreen">(Option équipée)</span></th>'
        + '</tr>' 
        + '<tr>'
			+ '<th>Free Credits</th>'
			...
			+ '<th ' + highlightOptionsAlreadyTaken('magicFurler') + '>Furler</th>'
			+ '<th ' + highlightOptionsAlreadyTaken('vrtexJacket') + '>Jacket</th>'
			+ '<th ' + highlightOptionsAlreadyTaken('comfortLoungePug') + '>Comfort</th>'
        ...
        + '<tr>'
			...
			+ '<td class="type">'+ race.legdata.optionPrices.magicFurler + '</td>'
			+ '<td class="type">'+ race.legdata.optionPrices.vrtexJacket + '</td>'
			+ '<td class="type">'+ race.legdata.optionPrices.comfortLoungePug + '</td>'
    ...
    function _handleResponseReceived(request, response, postData) {
        ...
        console.log (' ' )
        console.log('Reception event class : '+eventClass)
    ...
    if (eventClass == 'AccountDetailsRequest')
        { 
            ...
            console.log('  ')
            console.log(body)
            transmissionajax2 ('AccountDetailsRequest',body)
            //console.log ('test2 '+body.scriptData.res[0]['raceId'])
        }



       if (eventClass == 'LogEventRequest')
       {

        if (body.scriptData.boatsAtSea)
           
            {  
                // console.log(body.scriptData)
                transmissionajax2 ('getracesinfos',body.scriptData)
                //console.log ('test2 '+body.scriptData.res[0]['raceId'])
            }
       }
    //fin de modif  jph   
        ...
        if (eventClass == 'getboatinfos') {
            ...
            handleFleet(request, response, postData,map);
    ...
    function handleBoatInfo (response) {
        if (response) {
            ...
            try {
                ...
                var messagejph=message

                console.log ('messagejph '+messagejph)


                var pos={"lat":55,"lon":-10}
                ...
                if (message.bs) {
                    ...
                    if (currentUserId ==  message.bs._id.user_id) {
                        ...
                        // modif jph 
                        transmissionajax2 ('getownboatinfos',message.bs)
                        // miseajourcarte(message.bs)
                        ...
                        handleFleetBoatInfo(message.bs);
                         ...
                         // modif jph 
                        fleetboatinfos=message.bs          // fleetboatinfos est une variable globale jph

                        // file d'attente pour les appels faits avant que map ne soit dispo
                        // Gestion du cas ou la carte n est pas encore creee
                       
                        updatefleetJP(map,fleetboatinfos)
                        transmissionajax2 ('fleetboatinfos',message.bs)
                        //fin de modif 
				...
				if (message.track) {
					if (message.track._id.user_id == currentUserId) {
                        ...
                        console.log ('track'+JSON.parse(response.body))
                        transmissionajax2 ('track',message)
                        //fin de modif 
                ...
                if (message.ba) {
                    ...
                    // modif jph 
                    transmissionajax2 ('boatactions',message.ba)
                    //fin de modif 
    ...
    function handleFleet (request, response, postData,map) {
        if (response) {
            try {
    ...
    function handleOwnTrackInfo(message) {
        ...
        //modifjph
        updateMapMeL(race, message.track);
    }
    ...
    function handleLegInfo(message) {
        ...
        switchMap(race);
         ...
         // modif jph 
         console.log ('leginfos'+JSON.stringify(message))
         transmissionajax2 ('leginfos',message)
         //fin de modif 
	...
	function handleRace_SelectorData(response) {
		/* store polar */
        ...
        // modif jph 
        transmissionajax2 ('polaires',boatPolar)
        //fin de modif 
    }
    ...
    function setUp () {
		...
		cbvrouteur = document.getElementById("vrouteurcb");
        ...
        //modifjph
		document.getElementById("vrouteurcb").addEventListener("change", updateFleetFilter);
