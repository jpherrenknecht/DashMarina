...
function start (races, raceFleetMap, isDisplay) {
    if (running) {
        ...
        running = true,
        sendNMEA(races);
		console.log(settings.nmeaInterval);
    ...
    try {
        races.forEach(function (r) {
            if (r.curr) {
                ...
                // sendSentence(r.id, "$" + hdt + "*" + nmeaChecksum(hdt)); Supprimé Manel car erreur systématique
...
function formatRPM (m) {
	...
	var sailid = m.sail % 10;    // Manel : id voile actuelle, éventuellement recouverte
	// const sailTable = [0, "Jib", "Spi", "Stay", "LJ", "C0", "HG", "LG", 8, 9] 
    // var sailName = m.speedT.sail;  	// voile de recouvrement, ou actuelle. Hélas, Avalon ne lit pas cete info
	// var sailid = sailTable.indexOf(sailName); 
    s += ",E,";
    s += sailid;
    s += "," + stamina.toFixed(1) + ",0";
    s += ",A"
    return s;
}
